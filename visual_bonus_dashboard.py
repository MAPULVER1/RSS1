...
def visual_bonus_dashboard():
    ...
    combined = pd.merge(regular, bonus_totals, on="user", how="outer") \
                 .fillna(0) \
    ...
    timeline = logs.copy()
    timeline["date_only"] = timeline["timestamp"].dt.date
    timeline = timeline.groupby("date_only").size().reset_index(name="Submissions")
